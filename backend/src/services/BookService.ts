...
export class BookService {
  ...
  async getBooks(filters: BookSearchFilters): Promise<{ books: Book[]; total: number }> {
    const {
      title,
      author,
      genre,
    ...
    try {
      ...
      return {
        books: booksResult.rows as Book[],
        total,
      };
    } catch (error) {
  ...
  async getBookById(bookId: string): Promise<Book | null> {
    try {
      ...
      return result.rows.length > 0 ? (result.rows[0] as Book) : null;
    } catch (error) {
  ...
  async updateBook(bookId: string, updateData: UpdateBookData): Promise<Book | null> {
    const {
      isbn,
      title,
      author,
  ...
  async updateBookAvailability(bookId: string, availableCopies: number): Promise<Book | null> {
    try {
      const result = await query(
        `UPDATE books
